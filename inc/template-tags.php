...
function wpg_the_image_attachment($size = 'large', $limit = 0, $offset = 0) {
	...
	if ($images) {
		...
		$i = 0;
		echo '<div id="gallery-'. $post->ID .'" class="gallery gallery-columns-3">';
		...
		foreach ($images as $image) {
			if ($start <= $i and $i < $stop) {
			   ...
			   $image_attributes   = wp_get_attachment_image_src( $image->ID, $size );
				...
				printf('<figure class="gallery-item"><div class="gallery-icon"><a href="http://%1$s" target="_blank"><img src="%2$s" alt="" /></a></div></figure>',
						$image->post_excerpt,
						$image_attributes[0] 
				);
			} 
