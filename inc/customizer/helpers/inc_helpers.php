 ...
 */
function get_all_terms( $only_tax = false ){

	// Array with one builtin category
...
function social_net_link($format_string) {
	...
	foreach ($social as $value) {
		...
		if (get_theme_mod($key, '') !== '') {
			get_theme_mod($key);
			$links .= '<a href="'. get_theme_mod($key, '') . '"><i class="icon-'. $value .'"></i><span class="screen-reader-text">'. __('Profile on the site ','wpg_theme') . $value .'</span></a>';
		}
	}
	if ($links !== '') {
